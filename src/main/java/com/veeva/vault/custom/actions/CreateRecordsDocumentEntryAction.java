...
public class CreateRecordsDocumentEntryAction implements DocumentAction {
    ...
    public void execute(DocumentActionContext documentActionContext) {
    	...
    	Record documentTaskRecord = recordService.newRecord("vsdk_document_task__c");
    	documentTaskRecord.setValue("name__v", "vSDK Document Task " + Instant.now().toEpochMilli());
    	documentTaskRecord.setValue("document__c", id);
    	documentTaskRecord.setValue("assigned_to__c", requestContext.getInitiatingUserId());
    	recordList.add(documentTaskRecord);
        ...
        .onErrors(batchOperationErrors -> {
        	...
        	List<String> notificationUsers = VaultCollections.newList();
        	successMessage.stream().forEach(positionalRecordId -> {
	    		...
	    		notificationUsers.add(requestContext.getInitiatingUserId());
	            notificationParameters.setRecipientsByUsers(notificationUsers);
	            notificationParameters.setSenderId(requestContext.getInitiatingUserId());
